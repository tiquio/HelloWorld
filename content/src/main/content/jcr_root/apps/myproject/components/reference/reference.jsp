...
try {
    ...
    if (target != null) {
        %><div style="display:inline;" class="<%= ddClassName %>"><% needToCloseDiv = true; %>

            <%--<sling:include path="<%= target %>"/></div>--%>
            The parent node would be <b><%= target%></b> properties could be obtained from it by using ResourceResolver.
